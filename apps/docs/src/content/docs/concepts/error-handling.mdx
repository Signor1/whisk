...
All live in `@signordev/whisk-core/recovery` (server-friendly, no React
dependency) and are re-exported from `@signordev/whisk-core` for
convenience.
...
if (result.status === "complete") {
  // result.message + result.attestation are ready to submit
}
```
...
```ts
import { cctpDomainFor, chainForCctpDomain } from "@signordev/whisk-core";
```
...
async function bridgeWithRecovery({
  ...
  if (iris.status !== "complete" || !iris.message || !iris.attestation) {
    // Iris itself failed. Surface the snapshot so the user can resume
    // later — clearInflight is NOT called here. The widget would
