...
3. **The `@signordev/whisk-core` package.** No React. The engine is
   pure TypeScript.
...
<InstallCommand
  packages={["@signordev/whisk-core", "@circle-fin/adapter-circle-wallets"]}
/>
...
/**
 * Adapter — talks to Circle's Wallets API on your service's behalf.
 * The entity secret and API key never leave the server.
 */
const adapter = await createCircleWalletsAdapter({
  apiKey: process.env.CIRCLE_API_KEY!,
  entitySecret: process.env.CIRCLE_ENTITY_SECRET!,
  walletId: process.env.CIRCLE_WALLET_ID!, // the DCW that holds the USDC
});

/**
 * Engine — same constructor the widget uses internally. Chains array
 * limits the routes the engine will consider; trim it to the
 * networks your treasury actually operates on.
 */
...
/**
 * Send 100 USDC to a recipient. Same `quote → send` shape the
 * widget uses — the engine doesn't know or care that no React tree
 * is mounted.
 */
